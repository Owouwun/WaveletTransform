...
namespace wavelet_trans
{
	...
	enum wavelets
	{
		hermitian1 = 0, // Самый простой из описываемых вейвлетов. Тем не менее, один из лучших.
		hermitian2 = 1, // Соответствует вейвлету "мексиканская шляпа".
		hermitian3 = 2, // Показал себя лучше всех во время тестов.
		poisson2 = 3, // Показал себя хуже всех во время тестов.
		morlet = 4,
		modified_morlet = 5
	};

	// Функция, используемая во время тестирования. Возвращает массив, определённый на значениях от a до a+h*size c шагом h.
	double* testSignal(double a, double h, double size) {
		double* res = new double[size];
		const double frq1 = 7E2, amp1 = 1000;
		const double frq2 = 0, amp2 = 2;
		double t;
		double start = a + size * 0.3 * h;

		for (int i = 0; i < size * 0.3; i++)
			res[i] = 0;
		for (int i = size * 0.3; i < size * 0.776; i++) {
			t = a + i * h;
			res[i] = amp1 * sin(2 * PI * frq1 * (t - start)) * sin(2 * PI * frq1 * (t - start) * 5 * 2);
	...
	// Внимание! Если знвчение интеграла выйдет за пределы допустимого диапазона (будет равен inf), вернётся 0.
